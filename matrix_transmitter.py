...
conj = np.conjugate(pilot_state1)[::-1]
pilot_state1 = np.concatenate((pilot_state1, conj), axis=None)
pilot_state1 = np.insert(pilot_state1, 0, 0)
pilot_state1 = np.insert(pilot_state1, int(44100/2), 0)
...
'''
test fft.fft of a transmitted sine, try to get the channel coeff in the freq domain
do i need to ensure the transmitted pilots are conjugate symm
'''

print(pilot_state1[0])
print(pilot_state1[22050])

pilot1_freq = np.fft.fft(pilot_state1) # why is this not real????
print(pilot1_freq)
plt.plot(pilot1_freq)
plt.show()
